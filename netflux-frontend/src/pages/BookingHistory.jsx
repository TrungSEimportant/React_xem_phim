...
function BookingHistory({ user, navigateTo }) {
  ...
  useEffect(() => {
    if (user) {
      // Gọi API lấy vé của user hiện tại
      fetch(`http://localhost:5000/api/bookings/${user.name}`)
        .then(res => res.json())
        .then(data => {
           setHistoryList(data);
        })
          ...
          {historyList.map((ticket) => (
            ...
            >
              ...
              <div style={{ flex: '1', minWidth: '250px' }}>
                ...
                {/* Format lại ngày từ CSDL MySQL (created_at) */}
                <p style={{ margin: '5px 0', color: '#ccc' }}>Ngày thanh toán: <span>{new Date(ticket.created_at).toLocaleDateString('vi-VN')}</span></p>
              ...
              <div style={{ textAlign: 'right', minWidth: '180px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '10px' }}>
                <div>
                  <span style={{ 
                    backgroundColor: ticket.status === 'Thành công' ? 'rgba(46, 125, 50, 0.2)' : 'rgba(255, 152, 0, 0.2)', 
                    color: ticket.status === 'Thành công' ? '#4CAF50' : '#FF9800', 
                    border: ticket.status === 'Thành công' ? '1px solid #4CAF50' : '1px solid #FF9800', 
                    padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold' 
                  }}>
