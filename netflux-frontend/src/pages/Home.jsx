...
function Home({ movies, searchQuery, navigateTo, activeSection }) {
 ...
 const featuredMovie = useMemo(() => {
    // Nếu có phim hot, lấy ngẫu nhiên 1 phim trong mảng hotMovies
    if (hotMovies.length > 0) {
      const randomIndex = Math.floor(Math.random() * hotMovies.length);
      return hotMovies[randomIndex];
    }
    
    // Nếu không có phim hot nhưng có phim chung, lấy ngẫu nhiên 1 phim trong mảng movies
    if (movies.length > 0) {
      const randomIndex = Math.floor(Math.random() * movies.length);
      return movies[randomIndex];
    }
    
    return null; // Trả về null nếu không có phim nào để tránh lỗi
  }, [hotMovies, movies]);
